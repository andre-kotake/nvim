...
function M:is_client_configured(server_name, ft)
  ...
  for _, result in ipairs(active_autocmds) do
    if result.desc ~= nil and result.desc:match("server " .. server_name .. " ") then return true end
  end
  return false
end
...
function M:validate(server_name)
  ...
  if M:is_client_active(server_name) or M:is_client_configured(server_name) then return false end
...
function M:get_servers(servers)
  ...
  require("lazy.util").ls(vim.fn.stdpath("config") .. "/lua/plugins/lsp/servers/", function(_, name, type)
    if type == "file" then
      local file_name = name:gsub("%.%w+$", "")
      local server_opts = require("plugins.lsp.servers." .. file_name)

      if servers[file_name] ~= nil then server_opts = vim.tbl_deep_extend("force", servers[file_name], server_opts) end
    ...
    on_attach = function(client, bufnr)
      ...
      local buffer_mappings = {
        normal_mode = {
          ...
          ["gd"] = { "<cmd>lua vim.lsp.buf.definition()<CR>", "Go to [D]efinition" },
          ["gr"] = { "<cmd>lua vim.lsp.buf.references()<CR>", "Go to [R]eferences" },
      ...
      if type(server_opts.on_attach) == "function" then server_opts.on_attach(client, bufnr) end
    end,
    -- on_init = on_init,
    -- on_exit = on_exit,
...
function M.has_available_null_ls_formatters(bufnr)
  ...
  local null_ls_srcs = require("null-ls.sources").get_available(filetype, null_ls.methods.FORMATTING) or {}
