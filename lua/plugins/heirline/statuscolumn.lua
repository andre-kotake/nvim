...
local linenum = {
  ...
  provider = function(self)
    ...
    return string.rep("\u{00A0}", self.width - #lnum) .. lnum
  end,
...
local foldcolumn = {
  ...
  provider = function(self)
    ...
    local foldsign = (ok and fold ~= nil and fold) or empty
    return foldsign .. empty
  end,
  on_click = {
    callback = function(self)
      local function statuscolumn_clickargs()
        ...
        if args.char == empty or args.char == " " then
...
local signs = {
  ...
  provider = function(self)
    if self.sign then return empty .. self.sign.text end
    return " ┃ "
  end,
  hl = function(self)
