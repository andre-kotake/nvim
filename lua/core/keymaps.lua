...
local utils = require("core.utils.icons")

function M.get_desc(desc, icon)
  if not vim.g.enable_keymap_desc_icons then return desc end
  if type(icon) == "nil" then icon = desc end
  return utils.get(icon, true) .. " " .. desc
end

function M.setup()
  local map = vim.keymap.set
  local default_opts = function(desc, opts)
  ...
  map(
    ...
    "<leader>k",
    "<cmd>norm! K<cr>",
    default_opts("keywordprg", { expr = false })
  )
  ...
  map(
    ...
    default_opts("Save file with Ctrl+s")
  )

  map("n", "<leader>f", "", default_opts(M.get_desc("Find", "Find")))

  map("n", "<leader>b", "", default_opts(M.get_desc("Buffer", "Buffer")))
  map("n", "<leader>bd", "<cmd>bd<cr>", default_opts("Delete Buffer"))
  map("n", "<leader>bp", "<cmd>bp<cr>", default_opts("Previous Buffer"))

  map("n", "<leader>bn", "<cmd>bn<cr>", default_opts("Next Buffer"))
end
...
--   maps.x["P"] =
--     { "p", desc = "Yank what you are going to override, then paste" }
--
--   -- search highlighting ------------------------------------------------------
...
--     maps.n["<leader>pt"] =
--       { "<cmd>TSInstallInfo<cr>", desc = "Treesitter open" }
--   end
--
--   -- nvim updater
...
--     local is_filetype_allowed = vim.tbl_isempty(
--       autoformat.allow_filetypes or {}
...
--
--     if
--       is_autoformat_enabled
--       and is_filetype_allowed
--       and is_filetype_ignored
--     then
